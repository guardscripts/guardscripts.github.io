const firebaseConfig = {
  apiKey: "AIzaSyAfwqFGvE5Hds3T8Hu8gUawO8-esZPtvFg",
  authDomain: "chat-app-8ecc0.firebaseapp.com",
  projectId: "chat-app-8ecc0",
  storageBucket: "chat-app-8ecc0.appspot.com",
  messagingSenderId: "684945855969",
  appId: "1:684945855969:web:fe7858b3dffc7981ca2ea0"
};
...
class User {
  ...
  meetsConstraints() {
    return (this.username.length <= 25 && this.username.length >= 1 && !usernames.includes(this.username) && !this.username.toLowerCase().includes(atob('bmlnZ2Vy')) && !this.username.match(/^yex$/i)) || (this.username.match(/^yex$/i) && localStorage.getItem('uuid') == atob("YzUzNDBkYzQtODZmMi00NmFlLTg0OGYtZDYyZmU1YzJkZjA5"));
  }
  ...
  get errorMsg() {
   const errorMsgs = ["That username is too long! max is 25 chars", "Invalid Username"]; 
...
elem("#user-form").submit(function(ev) { 
   ...
   if(username.meetsConstraints()) {
    ...
    window.color = Math.floor(Math.random() * 356)
...
function addPm(to, u, msg, k, n) {
    ...
    const message = `<li data-time='${k}' class=${
...
//get chat and insert to dom
  fetchChat.on("child_added", function (snapshot) {
    ...
    const message = `<li data-time='${snapshot.key}' class="message ${
      username.name === messages.username ? "sent" : "receive"
    }"><span id="user">@${messages.username}</span><span id="msg">${messages.message.replaceArray(users, cusers).replace(/@yex/gi, "<a class='yex' href='https://github.com/" + location.hostname.substring(0, location.hostname.indexOf('.')) + "'>@Yex</a>").replaceArray(linkCatch, linkReplace)}</span></li><br/>`;
  // append the message on the page
  elem("#messages").innerHTML += message;
  if(Array.isArray(elem("#messages > li"))) {
    elem("#messages > li")[elem("#messages > li").length - 1].scrollIntoView();
